...
namespace RealEstateManagement.Application.Fields;
...
public sealed class FieldService(IFieldStore store, ISystemClock clock) : IFieldService
{
    ...
    private async Task<List<string>> ValidateEditorAsync(FieldEditorCommand command, Guid? fieldId, CancellationToken cancellationToken)
    {
        ...
        Required(command.Slug, "Vui lòng nhập Ä‘Æ°á»ng dẫn công khai.", errors);
        ...
        {
            errors.Add("ÄÆ°á»ng dẫn công khai tối đa 160 ký tự.");
        }

        if (command.MinimumBookingMinutes <= 0)
        {
            errors.Add("Thá»i lượng đặt tối thiểu phải lớn hơn 0.");
        }

        if (command.SlotStepMinutes <= 0)
        {
            errors.Add("Bước khung giá» phải lớn hơn 0.");
        }
        ...
        {
            errors.Add("ÄÆ°á»ng dẫn công khai đã tồn tại.");
        }
    ...
    private static void ValidateOperatingHours(IReadOnlyList<FieldOperatingHourCommand> hours, List<string> errors)
    {
        ...
        {
            errors.Add("Giá» hoạt động cần đủ 7 ngÃ y trong tuần.");
        }
        ...
        foreach (var hour in hours)
        {
            ...
            {
                errors.Add("NgÃ y trong tuần phải từ 0 đến 6.");
            }
            ...
            {
                errors.Add("Giá» hoạt động phải nằm trong khoảng 00:00 đến 24:00 vÃ  giá» mở nhá» hơn giá» đóng.");
            }
        }
    }
    ...
    private static void ValidatePricingRules(IReadOnlyList<PricingRuleCommand> rules, List<string> errors)
    {
        ...
        foreach (var rule in rules)
        {
            ...
            {
                errors.Add("Khung giá» giá phải hợp lệ.");
            }

            if (rule.PricePerHour < 0)
            {
                errors.Add("Giá theo giá» không được âm.");
            }

            if (rule.EffectiveTo is not null && rule.EffectiveTo < rule.EffectiveFrom)
            {
                errors.Add("NgÃ y kết thúc hiệu lực phải sau ngÃ y bắt đầu.");
            }

            if (rule.DayOfWeek is < 0 or > 6)
            {
                errors.Add("NgÃ y áp dụng giá phải từ 0 đến 6 nếu có nhập.");
            }
        }
    }
    ...
    private static List<string> ValidateBlock(FieldBlockCommand command)
    {
        ...
        {
            errors.Add("Khung giá» khóa sân không hợp lệ.");
        }

        Required(command.Reason, "Vui lòng nhập lý do khóa sân.", errors);
        if (command.CreatedByUserId == Guid.Empty)
        {
            errors.Add("Không xác định được ngÆ°á»i tạo lịch khóa sân.");
        }
